...
const CourseContent = ({
  ...
  return (
    <div>
      <Accordion.Header>
        <h1 className='mx-6 text-black text-xl font-medium'>{content.title}</h1>
      </Accordion.Header>
      <Accordion.Body>
        <div className='container my-2 pt-4'>
          <div className='bg-white px-lg-5 rounded-3'>
            ...
            <div>
              <h2 className='pb-5 italic'>{content.details}</h2>
              <p></p>
            </div>
              ...
              <ul className='list-disc flex flex-col'>
                ...
                {files &&
                  files.map((item, index) => (
                    <a href={`${item.file}.pdf`} target='_blank'>
                      Matarial {index + 1}
                    </a>
                  ))}
