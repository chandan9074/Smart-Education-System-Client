...
import { loadStudentsCourseContents, postStudentsCourseContent } from "../../services/auth";
...
const CourseDetails = () => {
  ...
  const fetchData = async () =>{
   ...
   const onFinish = async (values) => {
     ...
     const contentData = {title: title, details:details, links:links, courses:courseId.id};
     console.log(video);
    ...
    <div>
        ...
        <div className='container my-5 pt-4'>
          ...
          >
            <div className='my-auto'>
              ...
              <h4 className='text-xl font-medium'>Section: {courseData.classes[0].class_sec.section}</h4>
              <h4 className='text-xl font-medium'>
                Instructor: {courseData.instructor.user.first_name} {courseData.instructor.user.last_name}
              </h4>
            </div>
      ...
      <section className="container flex justify-center">
        {/* <form onSubmit={handleSubmit(onSubmit)} className="border-2 border-gray-150 p-5 flex flex-col w-3/4 rounded">
          ...
          <Form form={form} name="dynamic_form_nest_item" onFinish={onFinish} autoComplete="off" style={{ border:"1px solid #dbd7d7", padding:"60px", display:"flex", flexDirection:"column", width:"75%", borderRadius:5}}>
            ...
            >
              <Input style={{padding: 16, border: "2px solid #bfdbfe", borderRadius:4,   }} placeholder="Enter the content title" />
            </Form.Item>
        ...
        <Accordion>
          {contents.map((content, index) => (
            <Accordion.Item className='my-4 shadow-sm' eventKey={index}>
              <CourseContent content={content} onFetchData={fetchData} />
            </Accordion.Item>
          ))}
        </Accordion>
      </section>
