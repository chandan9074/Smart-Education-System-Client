...
import {
  ...
  loadContentsVideo,
  loadContentsFile,
  loadContentsFileDelete,
  loadContentsVideoDelete,
  updateStudentsCourseContent,
} from "../../services/auth";
...
const ContentUpdate = () => {
  ...
  const onFinish = async (values) => {
    ...
    const response = await updateStudentsCourseContent(
      contentAllData,
      video,
      files,
      contentData.id
    ...
    <div>
      ...
      <div className='container flex justify-center pt-32'>
        ...
        >
          ...
          <label style={{ fontSize: 21, fontWeight: 500, marginBottom: 8 }}>
            Title
          </label>
          <Form.Item
            name='title'
          ...
          {content &&
            content.map((item) => (
              <div className='flex items-center mb-2'>
                <span className='border border-gray-200 py-1 px-3 w-2/6 bg-white rounded mr-4'>
                  {item.title}
                </span>
                <span className='w-3/6 border border-gray-200 py-1 px-3 bg-white rounded mr-3'>
                  {item.video_link}
                </span>
                <MinusCircleOutlined
          ...
          {contentFiles &&
            contentFiles.map((cFile) => (
              <div className='border-2 border-gray-100 py-1 px-4 bg-green-50 mt-3 flex justify-between items-center'>
                ...
                <DeleteOutlined
                  className='cursor-pointer'
                  onClick={() => handleFileDelete(cFile.id)}
                />
              </div>
            ))}
